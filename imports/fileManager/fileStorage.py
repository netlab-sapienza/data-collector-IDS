...
class FileStorage(metaclass=Singleton):
    ...
    def __initializeDIR(self):
        ...
        dir_path = Path(script_root_path_str + "/results/" + str(sys.argv[1])).absolute()
        print("Checking if ./results/" + str(sys.argv[1]) + " exists...")
        try:
            os.makedirs(dir_path)
        except OSError as e:
            print("/results/" + str(sys.argv[1]) + " already exists... saving in this directory ")
